...
-- 5. Secure sensitive campaign data - drop existing policy first
DROP POLICY IF EXISTS "Everyone can view active campaigns" ON public.campaigns;

CREATE POLICY "Users can view campaigns with restrictions"
ON public.campaigns
FOR SELECT
USING (
  -- Campaign creators can see all their campaign details
  (auth.uid() = creator_id)
  OR 
  -- Public can only see basic info for active campaigns (no financial data)
...
-- 8. Add admin override policies for management
